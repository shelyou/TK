...
// Function to save updated price to Firestore
async function savePriceToFirestore(group, itemId, priceField, newPrice) {
    ...
    try {
        await updateDoc(docRef, { [priceField]: parseInt(newPrice) });
        alert("Price updated and saved to Firestore!");
    } catch (error) {
        console.error("Error updating price:", error);
        alert("Failed to save the updated price.");
    }
}
...
function editPrice(priceField, no, group) {
    ...
    if (newPrice && !isNaN(newPrice)) {
        // Find the item and update the price locally
        ...
        savePriceToFirestore(group, item.id, priceField, newPrice);
